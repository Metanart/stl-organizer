    ...
    try {
        ...
        for (const { path: basicPath } of paths) {
            ...
            try {
                ...
                const archives = files.filter((name) =>
                ...
                try {
                    ...
                    for (const [safeName, entry] of Object.entries(
                        folderResult.imagesMap,
                    )) {
                        const archiveFullPath =
                            folderResult.folderTree.children?.find(
                                (child) =>
                                    child.category === 'archive' &&
                                    child.name === entry.fileName,
                            )?.path;

                        if (!archiveFullPath) {
                            console.warn(
                                `Archive path not found for ${safeName}`,
                            );
                            continue;
                        }
                        ...
                        const prepared: PreparedModelDirectory =
                    ...
                    const transferredResultTree =
                        await scanFullTree(OUTPUT_ROOT);
