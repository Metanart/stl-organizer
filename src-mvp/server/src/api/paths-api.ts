...
export const getPathsHandler: RequestHandler = async (request: Request, response: Response) => {
  try {
...
export const addPathHandler: RequestHandler = async (request: Request, response: Response) => {
  const { path } = request.body

  if (typeof path !== 'string' || !path.trim()) {
    response.status(400).json({ message: 'Path must be a non-empty string' })
    return
  }
...
export const deletePathHandler: RequestHandler = async (request: Request, response: Response) => {
