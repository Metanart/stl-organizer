  ...
  if (category === 'folder') {
    ...
    for (const name of entries) {
      ...
      try {
        ...
        console.warn(`⚠️ Failed to scan ${childPath}:`, (err as Error).message)
      }
    }
    ...
    try {
      ...
      result.children = Object.values(entries).map((entry) => {
        ...
        return {
          ...
          type: isDir ? '' : fileExt,
          category,
          size: isDir ? null : parseFloat((entry.size / (1024 * 1024)).toFixed(2)),
      ...
      console.warn(`⚠️ Failed to read zip ${entryPath}:`, (err as Error).message)
    }
    ...
    try {
      ...
      for (const header of fileHeaders) {
        const fileExt = path.extname(header.name).slice(1).toLowerCase()
        ...
        result.children.push({
          ...
          children: null,
          windowsPath: toWindowsPath(path.join(entryPath, header.name))
        })
      }
    } catch (err) {
      console.warn(`⚠️ Failed to read rar ${entryPath}:`, (err as Error).message)
    }
  }
