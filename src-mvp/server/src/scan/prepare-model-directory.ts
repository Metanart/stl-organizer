...
export async function extractImagesFromRar(
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        process.on('close', (code) => {
            if (code !== 0) {
                return reject(
                    new Error(`unrar exited with code ${code}\n${stderr}`),
                );
            }
            resolve();
        });
...
async function copyImagesRecursive(
    fromDir: string,
    toDir: string,
): Promise<void> {
    ...
    try {
        ...
        if (archivePath !== archiveTarget) {
            try {
                ...
                console.warn(
                    `Failed to remove original archive: ${archivePath}`,
                    err,
                );
            }
        }
        ...
        for (const imgPath of externalImages) {
            ...
            try {
                ...
                console.warn(
                    `Failed to remove external image: ${imgPath}`,
                    err,
                );
            }
        }
